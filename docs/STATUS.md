...
**Last updated**: 2026-04-26 06:45 IST — final hackathon-criteria audit pass. **🎯 Submission lap CLOSED + Run 4 weights mirror is now LIVE on `anurag203/*`** (6.88 GB `model.safetensors` resolves to a presigned `cas-bridge.xethub.hf.co` URL with `x-xet-cas-uid=public`, so judges can download without auth). Run 4 eval (KL-anchor, β=0.2) is the central finding: event_planning μ 0.000 → **0.175** (beats 1.7B base 0.138), avg_score 0.029 → **0.056**, meeting peak 0.725 → 0.350 (the cost of breadth). All 3 trained runs are on W&B (<https://wandb.ai/anuragagarwal203-cisco/clarify-rl>), code is on GitHub (<https://github.com/anurag203/clarify-rl>), env Space at <https://huggingface.co/spaces/agarwalanu3103/clarify-rl>, demo Space at <https://huggingface.co/spaces/anurag203/clarify-rl-demo>. README has been restructured for judges (Judges-60s-Tour at top + Problem→Env→Results→Why-it-matters arc + 1-line caption under every plot + Wild Card #5 in the title block) **and now embeds a real demo-Space screenshot** (`assets/demo_replay_screenshot.png` — Replay viewer tab showing a Run 4 rollout with the per-rubric breakdown panel, served at HTTP 200 from raw.githubusercontent.com). Run 4 model card YAML cleaned to drop the `datasets: agarwalanu3103/clarify-rl` pill (that was the env Space, not a real dataset, and HF was rendering it as a disabled greyed-out tag). Logged-out smoke test of all 15 README-referenced URLs returns HTTP 200; HF Hub API confirms `cardData.datasets: ABSENT` and `tags with dataset prefix: []` on the cleaned Run 4 card. Env Space `/reset` returns a real `CallToolObservation` with task data. ~10.25 h to the 5 PM IST deadline.
...
- Deadline: Apr 26, 2026, 5:00 PM IST (~10.25 hours from this update)
