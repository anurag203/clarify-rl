...
**Last updated**: 2026-04-26 05:50 IST — final hackathon-criteria audit pass. **🎯 Submission lap CLOSED.** Run 4 eval (KL-anchor, β=0.2) is the central finding: event_planning μ 0.000 → **0.175** (beats 1.7B base 0.138), avg_score 0.029 → **0.056**, meeting peak 0.725 → 0.350 (the cost of breadth). All 3 trained runs are on W&B (<https://wandb.ai/anuragagarwal203-cisco/clarify-rl>), code is on GitHub (<https://github.com/anurag203/clarify-rl>), env Space at <https://huggingface.co/spaces/agarwalanu3103/clarify-rl>, demo Space at <https://huggingface.co/spaces/anurag203/clarify-rl-demo>. README has been restructured for judges (Judges-60s-Tour at top + Problem→Env→Results→Why-it-matters arc + 1-line caption under every plot + Wild Card #5 in the title block). Logged-out smoke test of all 25 submission URLs returns HTTP 200/302. Env Space `/reset` returns a real `CallToolObservation` with task data. ~11 h to the 5 PM IST deadline.
...
- 🟡 **Run 4 weights mirror to `anurag203/clarify-rl-run4-qwen3-1.7b-beta0.2`** — snapshot_download finished (6.88 GB safetensors fully on disk at `/tmp/clarify-rl-mirror/clarify-rl-run4-qwen3-1.7b-beta0.2/`), upload_folder phase currently at **26% uploaded (1.80 G / 6.88 G), ETA ~50 min** at observed 1.8 MB/s. Mirror PID 36249 is alive in background. **Not blocking submission** — every model card has a fallback `from_pretrained()` snippet pointing at the upstream repo (`2022uec1542/clarify-rl-grpo-qwen3-1-7b` for Run 4, `agarwalanu3103/*` for Runs 1/2), all upstream weights are public + smoke-tested, and the demo Space already streams from upstream so the live chat tab works today regardless of mirror state.
...
- Deadline: Apr 26, 2026, 5:00 PM IST (~11 hours from this update)
